...
def entry_for_flora(desc_model, notes_model, place_model, time_model, virtues_model, postnotes_model):
    try:
        ...
        entry_for_flora(desc_model, notes_model, place_model, time_model, virtues_model, postnotes_model)

#######################################################################

def create_fauna_markov(filename):
    with open(filename, encoding="utf8") as f:
        text = f.read()
    return markovify.Text(text)

def entry_for_fauna(fauna_model):
    name = "### " + random.sample([item['color'] for item in colors.crayola["colors"]], 1)[0] + " " + \
        random.sample(animals.common["animals"], 1)[0]
    
    desc = ""
    for i in range(random.randint(4, 13)):
        desc += fauna_model.make_sentence().capitalize() + " "
